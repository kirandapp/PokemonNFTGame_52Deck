...
contract PokemonGameV3 is Ownable, IERC721Receiver {
    ...
    function initialize(address _nft, address _token, address pokemonStatsAddress) public {
    ...
    function createBattle(uint256 _tokenamount) external returns (uint256) {
        ...
        for (uint256 i = 0; i < SELECT_CARD_FROM_DECK; i++) {
            ...
            if (statindex == 0) {
                stat += attack;
                selectedNftStats[i] = attack;
            } else if (statindex == 1) {
                stat += defense;
                selectedNftStats[i] = defense;
            } else if (statindex == 2) {
                stat += sp;
                selectedNftStats[i] = sp;
            } else if (statindex == 3) {
                stat += hp;
                selectedNftStats[i] = hp;
            }else {
                stat += mp;
                selectedNftStats[i] = mp;
            }
        }
    ...
    function setPokemonStats(uint256 tokenId, uint256 attack, uint256 defense, uint256 sp, uint256 hp, uint256 mp, uint _typeIndex) external {
        ...
        IPokemonStat(ps).setPokemonStats(tokenId, attack, defense, sp, hp, mp, _typeIndex);
    }

    //  GETTER FUNCTIONS
    function getPokemon(uint256 tokenId) public view returns (uint256, uint256, uint256, uint256, uint256) {
        // PokemonStats storage stats = _pokemonStats[tokenId];
        (uint256 attack, uint256 defense, uint256 sp, uint256 hp, uint256 mp,) = IPokemonStat(ps).getPokemonStats(tokenId);
        return (attack, defense, sp, hp, mp);  
    }
    ...
    function getStatsContract() public view returns (address) {
        return ps;
    }
