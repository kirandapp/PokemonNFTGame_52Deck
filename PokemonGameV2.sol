...
contract PokemonGame is Ownable, IERC721Receiver {
    ...
    function joinBattle(uint _matchId) external {
        ...
        require(TokenContract.balanceOf(msg.sender) >= bt.battleamount, "Insufficient Tokens Balance!");
        ...
        TokenContract.transferFrom(msg.sender, address(this), bt.battleamount);
        ...
        if (bt.stat > stat) {
            ...
            isplayerwin = false;
            sendReward(bt.battleamount, bt.creatorAddress);
            ...
            battleStatus[_matchId] = BattleStatus.Close;
            isplayerwin = true;
            sendReward(bt.battleamount, playeraddress);
        } else {
